...
const getFeatures = (theme: string): Feature[] => (
  [
    ...
    },
    {
      ...
      subheader: 'We aslo have included all the necessary layouts for a startup.',
      title: 'Landing Pages'
    }
  ]
);
...
const HomeFeatures: FC = (props) => {
  ...
  useEffect(() => {
    (
      async () => {
        const responses = await Promise.all(features.map((feature) => fetch(
          feature.image
        )));
            ...
            {features.map((feature, index) => {
              ...
              return (
                <Box
                  ...
                  sx={{
                    backgroundColor: index === selectedFeatureIndex && alpha(
